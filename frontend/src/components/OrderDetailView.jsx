...
export default function OrderDetailView({ order }) {
  ...
  return <>
    ...
    <Row className="g-4 order-detail-grid">
      <Col lg={8}>
        <section className="order-detail-section"><div className="detail-section-heading"><h2>Sản phẩm</h2><span>{order.items.length} mặt hàng</span></div><ListGroup variant="flush">{order.items.map((item, index) => <ListGroup.Item key={`${order.id}-${item.product_id}-${item.size || ''}-${item.color || ''}-${index}`} className="order-product">{item.thumbnail ? <Image src={item.thumbnail} alt={item.title} /> : <div className="summary-placeholder"><i className="bi bi-image" /></div>}<div><strong>{item.title}</strong>{(item.size || item.color) && <span>{item.size && `Size ${item.size}`}{item.size && item.color && ' · '}{item.color}</span>}<span>{formatCurrency(Number(item.price))} × {item.qty}</span></div><b>{formatCurrency(Number(item.price) * item.qty)}</b></ListGroup.Item>)}</ListGroup><div className="order-cost-row"><span>Phí vận chuyển</span><span>Miễn phí</span></div><div className="order-grand-total"><span>Tổng thanh toán</span><strong>{formatCurrency(Number(order.total))}</strong></div></section>
      </Col>
      <Col lg={4}>
